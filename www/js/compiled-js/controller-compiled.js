...
$('#checkout-page .modal').css("display","table");// check if the coupon is valid or not
_context142.next=14;return Promise.resolve($.ajax(// load the list of shipping zones
...
beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("Authorization","Basic "+utopiasoftware[utopiasoftware_app_namespace].accessor);},dataType:"json",timeout:240000,// wait for 4 minutes before timeout of request
processData:true,data:{code:$('#checkout-page #checkout-payment-voucher-code').val().trim()}}));case 14:couponsArray=_context142.sent;if(!(couponsArray.length==0)){_context142.next=17;break;}throw"error - no coupon found";case 17:// get a local/deep-clone copy of the page's checkout order object
...
_context142.next=22;return Promise.resolve($.ajax({url:utopiasoftware[utopiasoftware_app_namespace].model.appBaseUrl+('/wp-json/wc/v3/orders/'+localOrderObject.id),type:"put",contentType:"application/json",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("Authorization","Basic "+utopiasoftware[utopiasoftware_app_namespace].accessor);},dataType:"json",timeout:240000,// wait for 4 minutes before timeout of request
processData:false,data:JSON.stringify(localOrderObject)}));case 22:localOrderObject=_context142.sent;// update the page checkout order with the updated order from the server
...
_context142.next=27;return utopiasoftware[utopiasoftware_app_namespace].controller.checkoutPageViewModel.pageShow();case 27:_context142.next=40;break;case 29:_context142.prev=29;_context142.t0=_context142['catch'](10);// hide the page loader modal
$('#checkout-page .modal').css("display","none");// hide all previously displayed ej2 toast
$('.page-toast').get(0).ej2_instances[0].hide('All');$('.timed-page-toast').get(0).ej2_instances[0].hide('All');// display toast to show that an error
_toast22=$('.timed-page-toast').get(0).ej2_instances[0];_toast22.cssClass='error-ej2-toast';_toast22.timeOut=3000;_toast22.content='Coupon not applied. Invalid coupon code';_toast22.dataBind();_toast22.show();case 40:case'end':return _context142.stop();}}},_callee142,this,[[10,29]]);}));function applyCouponButtonClicked(){return _ref145.apply(this,arguments);}return applyCouponButtonClicked;}(),/**
...
newOrder=JSON.parse(JSON.stringify(selectedOrder));delete newOrder.id;delete newOrder.coupon_lines;// update the status of the new order to "pending"
newOrder.status="pending";// update the selectedOrder status remotely
_context165.next=10;return Promise.resolve($.ajax({url:utopiasoftware[utopiasoftware_app_namespace].model.appBaseUrl+'/wp-json/wc/v3/orders',type:"post",contentType:"application/json",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("Authorization","Basic "+utopiasoftware[utopiasoftware_app_namespace].accessor);},dataType:"json",timeout:240000,// wait for 4 minutes before timeout of request
processData:false,data:JSON.stringify(newOrder)}));case 10:newOrder=_context165.sent;_context165.next=13;return utopiasoftware[utopiasoftware_app_namespace].databaseOperations.loadData("user-details",utopiasoftware[utopiasoftware_app_namespace].model.encryptedAppDatabase);case 13:userDetails=_context165.sent.userDetails;_context165.next=16;return $('#app-main-navigator').get(0).pushPage("checkout-page.html",{data:{orderData:newOrder}});case 16:_context165.next=29;break;case 18:_context165.prev=18;_context165.t0=_context165['catch'](3);console.log(_context165.t0,"REORDER ERROR");// hide all previously displayed ej2 toast
$('.page-toast').get(0).ej2_instances[0].hide('All');$('.timed-page-toast').get(0).ej2_instances[0].hide('All');// display toast message
_toast31=$('.timed-page-toast').get(0).ej2_instances[0];_toast31.cssClass='error-ej2-toast';_toast31.timeOut=3000;_toast31.content='Placing new order failed. Please retry';_toast31.dataBind();_toast31.show();case 29:_context165.prev=29;// hide the page loader
$('#track-order-page .modal').css("display","none");return _context165.finish(29);case 32:case'end':return _context165.stop();}}},_callee165,this,[[3,18,29,32]]);})),0);case 13:case'end':return _context166.stop();}}},_callee166,this);}));function reorderButtonClicked(_x61){return _ref168.apply(this,arguments);}return reorderButtonClicked;}()},/**
