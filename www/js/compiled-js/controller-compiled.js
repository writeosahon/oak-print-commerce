...
$('#track-order-page .modal').css("display","table");// handle the tasks in a separate queue
window.setTimeout(_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee165(){var selectedOrder,newOrder,userDetails,_toast31;return regeneratorRuntime.wrap(function _callee165$(_context165){while(1){switch(_context165.prev=_context165.next){case 0:// get the selected order to be checked out
...
delete newOrder.id;newOrder.transaction_id="";newOrder.line_items.forEach(function(lineItem){delete lineItem.id;for(var index=0;index<lineItem.meta_data.length;index++){delete lineItem.meta_data[index].id;}});newOrder.tax_lines.forEach(function(item){delete item.id;for(var index=0;index<item.meta_data.length;index++){delete item.meta_data[index].id;}});newOrder.shipping_lines.forEach(function(item){delete item.id;for(var index=0;index<item.meta_data.length;index++){delete item.meta_data[index].id;}});newOrder.fee_lines.forEach(function(item){delete item.id;for(var index=0;index<item.meta_data.length;index++){delete item.meta_data[index].id;}});delete newOrder.coupon_lines;newOrder.set_paid=false;// update the status of the new order to "pending"
newOrder.status="pending";// update the selectedOrder status remotely
_context165.next=16;return Promise.resolve($.ajax({url:utopiasoftware[utopiasoftware_app_namespace].model.appBaseUrl+'/wp-json/wc/v3/orders',type:"post",contentType:"application/json",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("Authorization","Basic "+utopiasoftware[utopiasoftware_app_namespace].accessor);},dataType:"json",timeout:240000,// wait for 4 minutes before timeout of request
processData:false,data:JSON.stringify(newOrder)}));case 16:newOrder=_context165.sent;_context165.next=19;return utopiasoftware[utopiasoftware_app_namespace].databaseOperations.loadData("user-details",utopiasoftware[utopiasoftware_app_namespace].model.encryptedAppDatabase);case 19:userDetails=_context165.sent.userDetails;_context165.next=22;return $('#app-main-navigator').get(0).pushPage("checkout-page.html",{data:{orderData:newOrder}});case 22:_context165.next=35;break;case 24:_context165.prev=24;_context165.t0=_context165['catch'](3);console.log(_context165.t0,"REORDER ERROR");// hide all previously displayed ej2 toast
$('.page-toast').get(0).ej2_instances[0].hide('All');$('.timed-page-toast').get(0).ej2_instances[0].hide('All');// display toast message
_toast31=$('.timed-page-toast').get(0).ej2_instances[0];_toast31.cssClass='error-ej2-toast';_toast31.timeOut=3000;_toast31.content='Placing new order failed. Please retry';_toast31.dataBind();_toast31.show();case 35:_context165.prev=35;// hide the page loader
$('#track-order-page .modal').css("display","none");return _context165.finish(35);case 38:case'end':return _context165.stop();}}},_callee165,this,[[3,24,35,38]]);})),0);case 13:case'end':return _context166.stop();}}},_callee166,this);}));function reorderButtonClicked(_x61){return _ref168.apply(this,arguments);}return reorderButtonClicked;}()},/**
...
$('#completed-orders-page #completed-orders-page-orders-container').html(ordersContent);resolve(ordersArray.length);// resolve the promise with length of the ordersArray
     ...
     */pendingOrdersPageViewModel:{/**
         ...
         * event is triggered when page is initialised
         */pageInit:function pageInit(event){//function is used to initialise the page if the app is fully ready for execution
var loadPageOnAppReady=function(){var _ref175=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee172(){var userDetails,searchResultsArray,toast,_toast34;return regeneratorRuntime.wrap(function _callee172$(_context172){while(1){switch(_context172.prev=_context172.next){case 0:if(!(!ons.isReady()||utopiasoftware[utopiasoftware_app_namespace].model.isAppReady===false)){_context172.next=3;break;}setTimeout(loadPageOnAppReady,500);// call this function again after half a second
return _context172.abrupt('return');case 3:// listen for the back button event
...
$('#pending-orders-page-pull-hook-fab',event.originalEvent.pullHook).html('<ons-progress-circular indeterminate modifier="pull-hook"></ons-progress-circular>');break;}});_context172.prev=6;// hide the previously displayed orders info
...
_context172.next=11;return utopiasoftware[utopiasoftware_app_namespace].databaseOperations.loadData("user-details",utopiasoftware[utopiasoftware_app_namespace].model.encryptedAppDatabase);case 11:userDetails=_context172.sent.userDetails;_context172.next=14;return utopiasoftware[utopiasoftware_app_namespace].controller.pendingOrdersPageViewModel.loadOrders({"page":1,"per_page":20,"order":"desc","orderby":"date","customer":userDetails.id,"status":"pending"});case 14:searchResultsArray=_context172.sent;_context172.next=17;return utopiasoftware[utopiasoftware_app_namespace].controller.pendingOrdersPageViewModel.displayPageContent(searchResultsArray[0]);case 17:if(searchResultsArray[0].length==0){// no orders were found
// hide the page preloader
...
$('#pending-orders-page .modal').css("display","none");}_context172.next=33;break;case 20:_context172.prev=20;_context172.t0=_context172['catch'](6);// hide the page preloader
$('#pending-orders-page .page-preloader').css("display","none");// hide the previously displayed orders info
$('#pending-orders-page .row').css("display","none");// show the page loader
$('#pending-orders-page .modal').css("display","none");// hide all previously displayed ej2 toast
$('.page-toast').get(0).ej2_instances[0].hide('All');$('.timed-page-toast').get(0).ej2_instances[0].hide('All');// display toast to show that an error
_toast34=$('.timed-page-toast').get(0).ej2_instances[0];_toast34.cssClass='error-ej2-toast';_toast34.timeOut=3000;_toast34.content='Sorry, a search error occurred.'+(navigator.connection.type===Connection.NONE?" Connect to the Internet.":"");_toast34.dataBind();_toast34.show();case 33:case'end':return _context172.stop();}}},_callee172,this,[[6,20]]);}));return function loadPageOnAppReady(){return _ref175.apply(this,arguments);};}();var $thisPage=$(event.target);// get the current page shown
// call the function used to initialise the app page if the app is fully loaded
loadPageOnAppReady();},/**
         * method is triggered when page is shown
         */pageShow:function pageShow(){// update cart count
...
document.addEventListener("online",utopiasoftware[utopiasoftware_app_namespace].controller.pendingOrdersPageViewModel.deviceOnlineListener,false);},/**
         * method is triggered when page is hidden
         */pageHide:function(){var _ref176=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee173(){return regeneratorRuntime.wrap(function _callee173$(_context173){while(1){switch(_context173.prev=_context173.next){case 0:// remove listener for when the device does not have Internet connection
...
document.removeEventListener("online",utopiasoftware[utopiasoftware_app_namespace].controller.pendingOrdersPageViewModel.deviceOnlineListener,false);case 2:case'end':return _context173.stop();}}},_callee173,this);}));function pageHide(){return _ref176.apply(this,arguments);}return pageHide;}(),/**
         * method is triggered when page is destroyed
         */pageDestroy:function pageDestroy(){// reset the view-model properties
...
$('.page-toast').get(0).ej2_instances[0].hide('All');},/**
         ...
         * @param doneCallBack
         * @returns {Promise<void>}
         */pagePullHookAction:function(){var _ref177=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee174(){var doneCallBack=arguments.length>0&&arguments[0]!==undefined?arguments[0]:function(){};var userDetails,searchResultsArray,toast,_toast35;return regeneratorRuntime.wrap(function _callee174$(_context174){while(1){switch(_context174.prev=_context174.next){case 0:// disable pull-to-refresh widget till loading is done
$('#pending-orders-page #pending-orders-page-pull-hook').attr("disabled",true);// hide all previously displayed ej2 toast
$('.page-toast').get(0).ej2_instances[0].hide('All');_context174.prev=2;// show the page loader
$('#pending-orders-page .modal').css("display","table");// load the user profile details from the app database
_context174.next=6;return utopiasoftware[utopiasoftware_app_namespace].databaseOperations.loadData("user-details",utopiasoftware[utopiasoftware_app_namespace].model.encryptedAppDatabase);case 6:userDetails=_context174.sent.userDetails;_context174.next=9;return utopiasoftware[utopiasoftware_app_namespace].controller.pendingOrdersPageViewModel.loadOrders({"page":1,"per_page":20,"order":"desc","orderby":"date","customer":userDetails.id,"status":"pending"});case 9:searchResultsArray=_context174.sent;_context174.next=12;return utopiasoftware[utopiasoftware_app_namespace].controller.pendingOrdersPageViewModel.displayPageContent(searchResultsArray[0]);case 12:if(searchResultsArray[0].length==0){// no orders were found
// hide the page preloader
$('#pending-orders-page .page-preloader').css("display","none");// hide the previously displayed orders info
$('#pending-orders-page .row').css("display","none");// hide the page loader
$('#pending-orders-page .modal').css("display","none");// inform the user that no result for the search was found'
...
$('#pending-orders-page .page-preloader').css("display","none");// show the orders info
$('#pending-orders-page .row').css("display","block");// hide the page loader
$('#pending-orders-page .modal').css("display","none");}_context174.next=27;break;case 15:_context174.prev=15;_context174.t0=_context174['catch'](2);// hide the page preloader
$('#pending-orders-page .page-preloader').css("display","none");// show the page loader
$('#pending-orders-page .modal').css("display","none");// hide all previously displayed ej2 toast
$('.page-toast').get(0).ej2_instances[0].hide('All');$('.timed-page-toast').get(0).ej2_instances[0].hide('All');// display toast to show that an error
_toast35=$('.timed-page-toast').get(0).ej2_instances[0];_toast35.cssClass='error-ej2-toast';_toast35.timeOut=3000;_toast35.content='Sorry, a search error occurred.'+(navigator.connection.type===Connection.NONE?" Connect to the Internet.":"");_toast35.dataBind();_toast35.show();case 27:_context174.prev=27;// enable pull-to-refresh widget till loading is done
...
doneCallBack();return _context174.finish(27);case 32:case'end':return _context174.stop();}}},_callee174,this,[[2,15,27,32]]);}));function pagePullHookAction(){return _ref177.apply(this,arguments);}return pagePullHookAction;}(),/**
         * method is used to load orders to the page
         *
         * @param pageToAccess {Integer} the page within the paginated categories to retrieve
         *
         ...
         *
         * @returns {Promise<void>}
         */loadOrders:function(){var _ref178=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee175(queryParam){var pageToAccess=arguments.length>1&&arguments[1]!==undefined?arguments[1]:queryParam.page||1;var pageSize=arguments.length>2&&arguments[2]!==undefined?arguments[2]:queryParam.per_page||20;var promisesArray;return regeneratorRuntime.wrap(function _callee175$(_context175){while(1){switch(_context175.prev=_context175.next){case 0:queryParam.page=pageToAccess;queryParam.per_page=pageSize;promisesArray=[];// holds the array for the promises used to load the orders
// check if there is internet connection or not
if(navigator.connection.type!==Connection.NONE){// there is internet connection
// load the requested products list from the server
promisesArray.push(new Promise(function(resolve,reject){Promise.resolve($.ajax({url:utopiasoftware[utopiasoftware_app_namespace].model.appBaseUrl+"/wp-json/wc/v3/orders",type:"get",//contentType: "application/x-www-form-urlencoded",
...
promisesArray.push(Promise.reject("no internet connection"));}return _context175.abrupt('return',Promise.all(promisesArray));case 5:case'end':return _context175.stop();}}},_callee175,this);}));function loadOrders(_x70){return _ref178.apply(this,arguments);}return loadOrders;}(),/**
         * method is used to display the retrieved products on the search popover
         *
         * @param ordersArray
         *
         * @returns {Promise<void>}
         */displayPageContent:function(){var _ref179=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee176(ordersArray){var displayCompletedPromise;return regeneratorRuntime.wrap(function _callee176$(_context176){while(1){switch(_context176.prev=_context176.next){case 0:displayCompletedPromise=new Promise(function(resolve,reject){var ordersContent="";// holds the contents for the orders
// check if the ordersArray is empty or not
if(ordersArray.length<=0){// there are no new content to display
resolve(ordersArray.length);// resolve promise with the length of the orders array
}else{// there are some orders to display
...
}});return _context176.abrupt('return',displayCompletedPromise);case 2:case'end':return _context176.stop();}}},_callee176,this);}));function displayPageContent(_x71){return _ref179.apply(this,arguments);}return displayPageContent;}()}};
