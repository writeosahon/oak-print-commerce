...
$('#loader-modal').get(0).hide();return _context196.finish(110);case 117:return _context196.abrupt('return');case 118:case'end':return _context196.stop();}}},_callee196,this,[[18,55,66,73],[39,44],[79,99,110,117],[86,91]]);}));function reorderButtonClicked(){return _ref197.apply(this,arguments);}return reorderButtonClicked;}(),/**
         ...
         * @returns {Promise<void>}
         */displayContent:function(){var _ref200=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee199(){var orderData,shippingCountryCode,shippingZoneId,shippingCountry,shippingZone,shippingMethodsArray,shippingMethodDropDown,paymentMethodDropDown,couponsMultiSelectDropDown,couponsArray,orderItemsDisplayContent,index;return regeneratorRuntime.wrap(function _callee199$(_context199){while(1){switch(_context199.prev=_context199.next){case 0:_context199.prev=0;// get the order object set on this page
...
$('#order-details-page #order-details-list .order-details-order-number').html('#'+orderData.id);$('#order-details-page #order-details-list .order-details-order-status').html(''+orderData.status);// check if the order created (in GMT) has a pending 'Z' appended to the time
...
orderData.date_created_gmt+='Z';}$('#order-details-page #order-details-list .order-details-order-date').html(''+kendo.toString(new Date(orderData.date_created_gmt),"MMMM dd, yyyy"));$('#order-details-page #order-details-list .order-details-order-total').html('&#x20a6;'+kendo.toString(kendo.parseFloat(orderData.total),"n2"));$('#order-details-page #order-details-list .order-details-payment-method').html(''+orderData.payment_method_title);$('#order-details-page #order-details-list .order-details-shipping-method').html(''+orderData.shipping_lines[0].method_title);$('#checkout-page #checkout-billing-information-address').html(orderData.billing.address_1);$('#checkout-page #checkout-billing-information-city').html(orderData.billing.city);$('#checkout-page #checkout-shipping-information-first-name').html(orderData.shipping.first_name);$('#checkout-page #checkout-shipping-information-last-name').html(orderData.shipping.last_name);$('#checkout-page #checkout-shipping-information-address').html(orderData.shipping.address_1);$('#checkout-page #checkout-shipping-information-city').html(orderData.shipping.city);// get the shipping zone the of the user by checking the user's shipping country
shippingCountryCode=orderData.shipping.country==""?'NG':orderData.shipping.country;shippingZoneId=0;// set the shipping zone id to the default i.e. 'Rest of the world'
// find the country with the specified country code
shippingCountry=utopiasoftware[utopiasoftware_app_namespace].controller.checkoutPageViewModel.countryArray.find(function(countryElem){return countryElem.code===shippingCountryCode;});// check if a shipping country was discovered
if(shippingCountry){// a shipping country was discovered
...
//get the id of the discovered shipping zone
shippingZoneId=shippingZone.id;}}// get the shipping methods attached to the discovered shipping zone
_context199.next=21;return Promise.resolve($.ajax(// load the list of shipping zones
{url:utopiasoftware[utopiasoftware_app_namespace].model.appBaseUrl+('/wp-json/wc/v3/shipping/zones/'+shippingZoneId+'/methods'),type:"get",//contentType: "application/json",
beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("Authorization","Basic "+utopiasoftware[utopiasoftware_app_namespace].accessor);},dataType:"json",timeout:240000,// wait for 4 minutes before timeout of request
processData:true,data:{}}));case 21:shippingMethodsArray=_context199.sent;// filter the shipping methods for only the methods that are enabled
shippingMethodsArray=shippingMethodsArray.filter(function(shippingMethodElem){return shippingMethodElem.enabled===true;});// remove the "select" event listener for the shipping method dropdownlist
shippingMethodDropDown=$('#checkout-shipping-method-type').get(0).ej2_instances[0];shippingMethodDropDown.removeEventListener("select");// set the shippingMethodArray as the datasource for the shipping method dropdownlist
shippingMethodDropDown.dataSource=shippingMethodsArray;// set the pre-selected shipping method (i.e. the shippingMethod dropdownlist value)
// check if there are any shipping lines info available
...
$('#checkout-page .checkout-page-discount').css("display","block");}else{// the discount total is zero
// hide the discount total from user
$('#checkout-page .checkout-page-discount').css("display","none");}case 48:_context199.prev=48;return _context199.finish(48);case 50:case'end':return _context199.stop();}}},_callee199,this,[[0,,48,50]]);}));function displayContent(){return _ref200.apply(this,arguments);}return displayContent;}()}};
