...
_orderItemsDisplayContent='';// holds the content to be displayed for the order items segment
for(index=0;index<orderData.line_items.length;index++){_orderItemsDisplayContent+='<div class="col-xs-6" style="text-align: right; padding-right: 5px;\n                        padding-top: 10px; padding-bottom: 10px">'+orderData.line_items[index].name+'</div>\n                        <div class="col-xs-2" style="text-align: left;\n                        padding-top: 10px; padding-bottom: 10px">&times;'+orderData.line_items[index].quantity+'</div>\n                        <div class="col-xs-4" style="text-align: left; padding-left: 5px;\n                        padding-top: 10px; padding-bottom: 10px">\n                        &#x20a6;'+kendo.toString(kendo.parseFloat(orderData.line_items[index].subtotal),"n2")+'</div>';}$('#checkout-page #checkout-order-items-container').html(_orderItemsDisplayContent);// display checkout totals
...
utopiasoftware[utopiasoftware_app_namespace].controller.orderDetailsPageViewModel.orderDetails=$('#app-main-navigator').get(0).topPage.data.orderData;_context193.prev=5;// create the "Reorder" button
new ej.splitbuttons.ProgressButton({cssClass:'e-hide-spinner',duration:10*60*60*1000// set spinner/progress duration for 10 hr
...
$('#app-main-navigator').get(0).popPage();},/**
         ...
         *
         * @returns {Promise<void>}
         */reorderButtonClicked:function(){var _ref199=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee198(){var toast,userDetails,payStackResponse,transactionCompletedUrl,searchParams,completedTransactionReference,localOrderObject,_toast42,_localOrderObject2,_toast43;return regeneratorRuntime.wrap(function _callee198$(_context198){while(1){switch(_context198.prev=_context198.next){case 0:if(!(navigator.connection.type===Connection.NONE)){_context198.next=14;break;}// there is no Internet connection
// hide all previously displayed ej2 toast
$('.page-toast').get(0).ej2_instances[0].hide('All');$('.timed-page-toast').get(0).ej2_instances[0].hide('All');// display toast to show that an error
toast=$('.timed-page-toast').get(0).ej2_instances[0];toast.cssClass='error-ej2-toast';toast.timeOut=3000;toast.content='Connect to the Internet to make payment';toast.dataBind();toast.show();// enable the "Make Payment" button
$('#checkout-page #checkout-make-payment').removeAttr("disabled");// hide the spinner from the 'Make Payment'
$('#checkout-page #checkout-make-payment').get(0).ej2_instances[0].cssClass='e-hide-spinner';$('#checkout-page #checkout-make-payment').get(0).ej2_instances[0].dataBind();$('#checkout-page #checkout-make-payment').get(0).ej2_instances[0].stop();return _context198.abrupt('return');case 14:if(!(utopiasoftware[utopiasoftware_app_namespace].controller.checkoutPageViewModel.chekoutOrder.payment_method==="paystack")){_context198.next=74;break;}// user selected the paystack (Pay With Card)
// disable the "Make Payment" button
$('#checkout-page #checkout-make-payment').attr("disabled",true);// inform the user that payment gateway is being prepared
$('#loader-modal-message').html("Preparing Payment Channel...");$('#loader-modal').get(0).show();// show loader
_context198.prev=18;_context198.next=21;return utopiasoftware[utopiasoftware_app_namespace].databaseOperations.loadData("user-details",utopiasoftware[utopiasoftware_app_namespace].model.encryptedAppDatabase);case 21:userDetails=_context198.sent.userDetails;_context198.next=24;return Promise.resolve($.ajax({url:'https://api.paystack.co/transaction/initialize',type:"post",contentType:"application/json",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("Authorization","Bearer "+Base64.decode(utopiasoftware[utopiasoftware_app_namespace].paystackAccessor));},dataType:"json",timeout:240000,// wait for 4 minutes before timeout of request
processData:false,data:JSON.stringify({email:userDetails.email,amount:""+kendo.parseFloat(utopiasoftware[utopiasoftware_app_namespace].controller.checkoutPageViewModel.chekoutOrder.total)*100,callback_url:"https://shopoakexclusive.com/"})}));case 24:payStackResponse=_context198.sent;if(!(payStackResponse.status===true)){_context198.next=52;break;}_context198.next=28;return new Promise(function(resolve,reject){// create/open inapp browser
var transactionInAppBrowser=cordova.InAppBrowser.open(window.encodeURI(payStackResponse.data.authorization_url),'_blank','location=yes,clearcache=yes,clearsessioncache=yes,closebuttoncolor=#ffffff,hardwareback=no,hidenavigationbuttons=yes,hideurlbar=yes,zoom=no,toolbarcolor=#3f51b5');// add event listeners for the transaction inapp browswer
transactionInAppBrowser.addEventListener("loadstart",function(loadStartEvent){// check which url is being loaded
if(loadStartEvent.url.startsWith("https://shopoakexclusive.com/")){// transaction was completed
// set a flag to indicate that the transaction was completed
...
resolve(transactionInAppBrowser._utopiasoftware_transaction_completed_url);}else{// transaction was not completed
reject();// reject parent promise
}});});case 28:transactionCompletedUrl=_context198.sent;// get the search parameters object from the transaction completed url
searchParams=new URLSearchParams(transactionCompletedUrl.split("?")[1]);// get the 'reference' search parameter value
completedTransactionReference=searchParams.get("reference");// inform the user that their order is being placed
$('#loader-modal-message').html("Completing Order Placement...");// get a local/deep-clone copy of the page's checkout order object
localOrderObject=JSON.parse(JSON.stringify(utopiasoftware[utopiasoftware_app_namespace].controller.checkoutPageViewModel.chekoutOrder));// update the order status (by setting the order paid flag) and transaction reference
...
processData:false,data:JSON.stringify(localOrderObject)}));case 38:localOrderObject=_context198.sent;_context198.prev=39;_context198.next=42;return utopiasoftware[utopiasoftware_app_namespace].databaseOperations.removeData("user-cart",utopiasoftware[utopiasoftware_app_namespace].model.appDatabase);case 42:_context198.next=46;break;case 44:_context198.prev=44;_context198.t0=_context198['catch'](39);case 46:// update the checkout-order-placement-modal with the checkout order number
$('#checkout-order-placement-modal .order-number').html(localOrderObject.number);// add the click handler for the 'checkout-order-placement-modal-ok-button'
$('#checkout-order-placement-modal #checkout-order-placement-modal-ok-button').get(0).onclick=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee196(){return regeneratorRuntime.wrap(function _callee196$(_context196){while(1){switch(_context196.prev=_context196.next){case 0:_context196.next=2;return $('ons-splitter').get(0).content.load("app-main-template");case 2:_context196.next=4;return $('#checkout-order-placement-modal').get(0).hide();case 4:case'end':return _context196.stop();}}},_callee196,this);}));// show the 'checkout-order-placement-modal'
_context198.next=50;return $('#checkout-order-placement-modal').get(0).show();case 50:_context198.next=53;break;case 52:throw"error";case 53:_context198.next=66;break;case 55:_context198.prev=55;_context198.t1=_context198['catch'](18);console.log("PAYMENT ERROR",_context198.t1);// hide all previously displayed ej2 toast
$('.page-toast').get(0).ej2_instances[0].hide('All');$('.timed-page-toast').get(0).ej2_instances[0].hide('All');// display toast to show that an error
_toast42=$('.timed-page-toast').get(0).ej2_instances[0];_toast42.cssClass='error-ej2-toast';_toast42.timeOut=3500;_toast42.content='Error making payment for this order. Try again';_toast42.dataBind();_toast42.show();case 66:_context198.prev=66;// enable the "Make Payment" button
$('#checkout-page #checkout-make-payment').removeAttr("disabled");// hide the spinner from the 'Make Payment'
$('#checkout-page #checkout-make-payment').get(0).ej2_instances[0].cssClass='e-hide-spinner';$('#checkout-page #checkout-make-payment').get(0).ej2_instances[0].dataBind();$('#checkout-page #checkout-make-payment').get(0).ej2_instances[0].stop();// hide loader
$('#loader-modal').get(0).hide();return _context198.finish(66);case 73:return _context198.abrupt('return');case 74:if(!(utopiasoftware[utopiasoftware_app_namespace].controller.checkoutPageViewModel.chekoutOrder.payment_method==="cod")){_context198.next=118;break;}// user selected the cod (Cash on Delivery)
// disable the "Make Payment" button
$('#checkout-page #checkout-make-payment').attr("disabled",true);// inform the user that their order is being placed
$('#loader-modal-message').html("Completing Order Placement...");_context198.next=79;return $('#loader-modal').get(0).show();case 79:_context198.prev=79;// get a local/deep-clone copy of the page's checkout order object
...
processData:false,data:JSON.stringify(_localOrderObject2)}));case 85:_localOrderObject2=_context198.sent;_context198.prev=86;_context198.next=89;return utopiasoftware[utopiasoftware_app_namespace].databaseOperations.removeData("user-cart",utopiasoftware[utopiasoftware_app_namespace].model.appDatabase);case 89:_context198.next=93;break;case 91:_context198.prev=91;_context198.t2=_context198['catch'](86);case 93:// update the checkout-order-placement-modal with the checkout order number
...
_context198.next=97;return $('#checkout-order-placement-modal').get(0).show();case 97:_context198.next=110;break;case 99:_context198.prev=99;_context198.t3=_context198['catch'](79);console.log("PAYMENT ERROR",_context198.t3);// hide all previously displayed ej2 toast
$('.page-toast').get(0).ej2_instances[0].hide('All');$('.timed-page-toast').get(0).ej2_instances[0].hide('All');// display toast to show that an error
_toast43=$('.timed-page-toast').get(0).ej2_instances[0];_toast43.cssClass='error-ej2-toast';_toast43.timeOut=3500;_toast43.content='Error placing this order. Try again';_toast43.dataBind();_toast43.show();case 110:_context198.prev=110;// enable the "Make Payment" button
$('#checkout-page #checkout-make-payment').removeAttr("disabled");// hide the spinner from the 'Make Payment'
$('#checkout-page #checkout-make-payment').get(0).ej2_instances[0].cssClass='e-hide-spinner';$('#checkout-page #checkout-make-payment').get(0).ej2_instances[0].dataBind();$('#checkout-page #checkout-make-payment').get(0).ej2_instances[0].stop();// hide loader
$('#loader-modal').get(0).hide();return _context198.finish(110);case 117:return _context198.abrupt('return');case 118:case'end':return _context198.stop();}}},_callee198,this,[[18,55,66,73],[39,44],[79,99,110,117],[86,91]]);}));function reorderButtonClicked(){return _ref199.apply(this,arguments);}return reorderButtonClicked;}(),/**
         * method is used to load the order details data into the page
         * @returns {Promise<void>}
         */displayContent:function(){var _ref202=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee199(){var orderData,_displayContent,index,orderNote,_index;return regeneratorRuntime.wrap(function _callee199$(_context199){while(1){switch(_context199.prev=_context199.next){case 0:try{// get the order object set on this page
