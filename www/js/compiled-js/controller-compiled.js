...
_context55.next=3;return $('#third-party-login-modal').get(0).hide();case 3:if(!(loginMode==="sign in")){_context55.next=43;break;}_context55.prev=4;// display modal to user that signin is being completed
...
beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("Authorization","Basic "+utopiasoftware[utopiasoftware_app_namespace].accessor);},dataType:"json",timeout:240000,// wait for 4 minutes before timeout of request
processData:true,data:{email:authResult.user.email}}));case 10:resultArray=_context55.sent;if(!(resultArray.length==0)){_context55.next=13;break;}throw"error";case 13:// create a pseudo account for the 3rd-party login
try{Promise.resolve($.ajax({url:utopiasoftware[utopiasoftware_app_namespace].model.appBaseUrl+"/wp-json/wc/v3/customers",type:"post",contentType:"application/json",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("Authorization","Basic "+utopiasoftware[utopiasoftware_app_namespace].accessor);},dataType:"json",timeout:240000,// wait for 4 minutes before timeout of request
processData:false,data:JSON.stringify({email:$('#login-page #signup-form #signup-email').val().trim(),username:$('#login-page #signup-form #signup-email').val().trim(),password:$('#login-page #signup-form #signup-password').val().trim()})}));//todo
}catch(err){}// save the created user details data to ENCRYPTED app database as cached data
_context55.next=16;return utopiasoftware[utopiasoftware_app_namespace].databaseOperations.saveData({_id:"user-details",docType:"USER_DETAILS",userDetails:resultArray[0]},utopiasoftware[utopiasoftware_app_namespace].model.encryptedAppDatabase);case 16:_context55.next=18;return $('#loader-modal').get(0).hide();case 18:_context55.next=20;return utopiasoftware[utopiasoftware_app_namespace].controller.loginPageViewModel.backButtonClicked();case 20:// hide all previously displayed ej2 toast
$('.page-toast').get(0).ej2_instances[0].hide('All');$('.timed-page-toast').get(0).ej2_instances[0].hide('All');// display toast message
_toast=$('.timed-page-toast').get(0).ej2_instances[0];_toast.cssClass='success-ej2-toast';_toast.timeOut=3000;_toast.content='User signin completed';_toast.dataBind();_toast.show();_context55.next=43;break;case 30:_context55.prev=30;_context55.t0=_context55['catch'](4);console.log("SIGN IN ERROR",_context55.t0);// hide loader
_context55.next=35;return $('#loader-modal').get(0).hide();case 35:// hide loader
// hide all previously displayed ej2 toast
$('.page-toast').get(0).ej2_instances[0].hide('All');$('.timed-page-toast').get(0).ej2_instances[0].hide('All');// display toast message
_toast2=$('.timed-page-toast').get(0).ej2_instances[0];_toast2.cssClass='error-ej2-toast';_toast2.timeOut=3000;_toast2.content='User has not created an account yet. User signin failed ';_toast2.dataBind();_toast2.show();case 43:if(loginMode==="sign up"){}return _context55.abrupt('return',false);case 45:case'end':return _context55.stop();}}},_callee55,this,[[4,30]]);}));function signInSuccessWithAuthResult(_x24){return _ref56.apply(this,arguments);}return signInSuccessWithAuthResult;}()}});// disable firebase ui auto-signin
...
beforeSend:function beforeSend(jqxhr){if(userDetails.password){jqxhr.setRequestHeader("Authorization","Basic "+Base64.encode(userDetails.email+':'+userDetails.password));}else{// use the developer admin credentials instead
jqxhr.setRequestHeader("Authorization","Basic "+utopiasoftware[utopiasoftware_app_namespace].accessor);}},crossDomain:true,xhrFields:{withCredentials:true},dataType:"text",timeout:240000,// wait for 4 minutes before timeout of request
processData:true,// send the shipping method data represented by selected shipping method value
data:{id:shippingMethodDropDown.getDataByValue(shippingMethodDropDown.value).instance_id,method_id:shippingMethodDropDown.getDataByValue(shippingMethodDropDown.value).method_id}}));case 18:_context152.next=20;return Promise.resolve($.ajax({url:utopiasoftware[utopiasoftware_app_namespace].model.appBaseUrl+'/wp-json/wc/v2/cart/calculate',type:"post",contentType:"application/json",beforeSend:function beforeSend(jqxhr){if(userDetails.password){jqxhr.setRequestHeader("Authorization","Basic "+Base64.encode(userDetails.email+':'+userDetails.password));}else{// use the developer admin credentials instead
jqxhr.setRequestHeader("Authorization","Basic "+utopiasoftware[utopiasoftware_app_namespace].accessor);}},crossDomain:true,xhrFields:{withCredentials:true},dataType:"text",timeout:240000,// wait for 4 minutes before timeout of request
...
jqxhr.setRequestHeader("Authorization","Basic "+utopiasoftware[utopiasoftware_app_namespace].accessor);}},crossDomain:true,xhrFields:{withCredentials:true},dataType:"json",timeout:240000,// wait for 4 minutes before timeout of request
